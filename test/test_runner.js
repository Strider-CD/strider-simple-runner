...
describe('Runner', function () {
  ...
  beforeEach(function (done) {
    processJob = function () {
      processing = [].slice.call(arguments);
    };
  ...
  it('should have correct options passed through to processJob', function (done) {
    ...
    runner.loadExtensions([], function () {
      runner.queueJob(
        { _id: 'id1', project: { name: 'man', provider: { id: 'test' } } },
        { runner: { id: 'simple-runner' }, plugins: [] }
      );
    });
  });
  ...
  describe('with a few queued jobs', function () {
    ...
    beforeEach(function (done) {
      ...
      ids.forEach(function (id) {
        runner.queueJob(
          { _id: id, project: { name: 'man', provider: { id: 'test' } } },
          { runner: { id: 'simple-runner' }, plugins: [] }
        );
      });
      done();
