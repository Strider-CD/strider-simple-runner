...
function run(io, cwd, command, next) {
  ...
  if (options.pty) {
    ...
    try {
      ...
      if (err.message !== 'forkpty(3) failed') {
...
function runSimple(io, cwd, command, args, next) {
  ...
  if (screencmd !== false) {
    io.emit('stdout', screencmd || command, 'command')
  }
...
function runPty(io, cwd, command, next) {
  ...
  var proc = pty.spawn(command, options, function(exitCode, data) {
    if (exitCode === 503) {
      io.emit('stderr', 'Unable to process exit code for command: ' + (options.screencmd || command), 'error')
      io.emit('log', 'Unable to process exit code', [data])
    }
    next(exitCode, data)
  })
  var first = true
