...
function initJobDirs(base, job, done) {
...
Runner.prototype = {
  ...
  plugins: function (user, config, job, dirs, done) {
    ...
    var tasks = [function (next) {
      ...
      if (!plugin.hosted) {
        return plugin.init(dirs, job.project.provider.config, job, finished)
      }
      plugin.init(dirs, user.account(job.project.provider).config, job.project.provider.config, job, finished)
    }]
  ...
  processJob: function (data, next) {
    var job = data.job
      ...
      , now = new Date()
      , self = this
    initJobDirs(this.config.dataDir, job, function (err, dirs) {
      if (err) {
