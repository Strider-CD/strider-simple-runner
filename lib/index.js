...
function makeCache(cacheDir) {
  return {
    get: function (dataDir, done) {
      cpDir(cacheDir, dataDir, done)
    },
    update: function (dataDir, done) {
      cpDir(dataDir, cacheDir, done)
    }
  }
}
...
// the job object.
// done(err, {base:, data:, cache:}, cacheExists)
function initJobDirs(base, job, useCache, done) {
  var name = job.project.name
    , dirs = {
        base: path.join(base, name, job._id.toString()),
        data: path.join(base, name, job._id.toString(), 'code'),
        cache: path.join(base, name, 'cache')
      }
  ...
  async.series([
    ...
    done(err, dirs, useCache && !results[2])
  })
}
...
Runner.prototype = {
  ...
  // cached - tell the provider to just update, b/c we restored the data from the cache
  plugins: function (user, config, job, dirs, cached, done) {
    var self = this
    ...
    var tasks = [function (next) {
      ...
      if (!plugin.hosted) {
        return plugin.init(dirs, cached, job.project.provider.config, job, finished)
      }
      plugin.init(dirs, cached, user.account(job.project.provider).config, job.project.provider.config, job, finished)
    }]
  ...
  processJob: function (data, next) {
    var job = data.job
      ...
      , now = new Date()
      , self = this
    initJobDirs(this.config.dataDir, job, config.runner.config.useCache, function (err, dirs, cached) {
      if (err) {
      ...
      self.plugins(job.project.creator, config, job, dirs, cached && makeCache(path.join(dirs.cache, 'code')), function (err, workers) {
        if (err) {
          var jobdata = self.jobdata.pop(job._id)
          jobdata.errored = true
          jobdata.error = {
