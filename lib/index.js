...
function initJobDirs(base, job) {
  ...
  var name = job.project.name.replace('/', '.') + '-' + job._id
    ...
    , dataDir = path.join(baseDir, 'cache')
...
Runner.prototype = {
  ...
  plugins: function (user, config, job, dirs, done) {
    ...
    var tasks = [function (next) {
      ...
      plugin.init(dirs, user.account(job.project.provider), job.project.provider.config, job, finished)
    }]
  ...
  processJob: function (data, next) {
    var job = data.job
      , config = data.config
      , dirs = initJobDirs(this.config.dataDir, job)
      , now = new Date()
      , self = this
