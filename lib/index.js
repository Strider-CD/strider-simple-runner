...
Runner.prototype = {
  ...
  plugins: function (user, config, job, dirs, done) {
    ...
    var tasks = [function (next) {
      ...
      plugin.init(dirs[1], user.providers[job.project.provider.id], job.project.provider.config, job, t(function (err, provider) {
    ...
    config.plugins.forEach(function (plugin) {
      ...
      tasks.push(function (next) {
        ...
        else fn(user.jobplugins[plugin.id], plugin.config, job, context, cb)
      })
    })
  ...
  processJob: function (data, next) {
    ...
    this.plugins(job.project.creator, config, job, dirs, function (err, workers) {
      ...
      console.log('going to process', dirs[0])
