...
function processPhases(done, buildHooks) {
  return function (err, result, results) {
    var next = this
      ...
      , runPhase = {}
      , runList = []
    if (err === 'no rules matched') {
      context.io.emit('stderr', 'No plugins found to test this project. For more information, see https://github.com/Strider-CD/strider/wiki/List-of-Plugins', 'error')
      return done(404)
    }
    if (err) return next(err)
